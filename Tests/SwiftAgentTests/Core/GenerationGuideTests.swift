...
struct GenerationGuideTests {
    ...
    @Test func decimalFactoriesSetBounds() throws {
        let lowerBound = try #require(Decimal(string: "2.5"))
        let upperBound = try #require(Decimal(string: "8.5"))
        let minimum = GenerationGuide<Decimal>.minimum(try #require(Decimal(string: "1.5")))
        let maximum = GenerationGuide<Decimal>.maximum(try #require(Decimal(string: "9.5")))
        let range = GenerationGuide<Decimal>.range(lowerBound ... upperBound)
