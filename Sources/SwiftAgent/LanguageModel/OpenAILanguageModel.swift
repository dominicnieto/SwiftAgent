...
public struct OpenAILanguageModel: LanguageModel {
    ...
    /// Normalized capabilities for the selected OpenAI-compatible API path.
    public var capabilities: LanguageModelCapabilities {
        var provider: ProviderCapabilities = [
            .textStreaming,
            .structuredOutputs,
            .toolCalling,
            .parallelToolCalls,
        ...
        if apiVariant == .responses {
            provider.insert(.toolCallStreaming)
            provider.insert(.reasoningSummaries)
            provider.insert(.encryptedReasoningContinuity)
            provider.insert(.responseContinuation)
            provider.insert(.streamingTokenUsage)
        }
        ...
        return LanguageModelCapabilities(
            model: ModelCapabilities(
                ...
                supportsReasoning: apiVariant == .responses
            ),
            provider: provider
        )
    }
    ...
    private func respondWithChatCompletions<Content>(
        ...
        while true {
            ...
            let resp: ChatCompletions.Response = try await httpClient.send(
    ...
    private func respondWithResponses<Content>(
        ...
        var messages = messages
            ...
            let resp: Responses.Response = try await httpClient.send(
                path: "responses",
                method: .post,
                queryItems: nil,
                headers: nil,
    ...
    fileprivate func toOpenAIMessages() -> [OpenAIMessage] {
        ...
        for item in self {
            ...
            case .toolOutput(let toolOutput):
                messages.append(
                    .init(
                        role: .tool(id: toolOutput.callId),
    ...
    for segment in segments {
        ...
        case .image(let image):
            ...
            case .data(let data, let mimeType):
                blocks.append(.imageURL("data:\(mimeType);base64,\(data.base64EncodedString())"))
            }
        }
    }
    return blocks
...
private enum OpenAIResponsesServerEvent: Decodable, Sendable {
    ...
    case outputItemAdded(OpenAIResponseOutputItem)
    ...
    init(from decoder: any Decoder) throws {
        ...
        case "response.output_item.added", "response.output_item.done":
            self = .outputItemAdded(try container.decode(OpenAIResponseOutputItem.self, forKey: .item))
...
private struct OpenAIResponseOutputItem: Decodable, Sendable {
    let id: String
    let type: String
    let name: String?
    let arguments: String?
