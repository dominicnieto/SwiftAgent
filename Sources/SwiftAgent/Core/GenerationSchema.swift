...
public struct GenerationSchema: Equatable, Codable, CustomDebugStringConvertible, Sendable {
    indirect enum Node: Equatable, Codable, Sendable {
        ...
        static func == (lhs: GenerationSchema.Node, rhs: GenerationSchema.Node) -> Bool {
            ...
            case (.object(let lhsObject), .object(let rhsObject)):
                return lhsObject.title == rhsObject.title
                    && lhsObject.description == rhsObject.description
                    && lhsObject.required == rhsObject.required
                    && lhsObject.orderedProperties == rhsObject.orderedProperties
        ...
        func encode(to encoder: Encoder) throws {
            ...
            case .object(let obj):
                ...
                try container.encode(obj.orderedProperties.filter { obj.required.contains($0) }, forKey: .required)
                try container.encode(obj.orderedProperties, forKey: .order)
        ...
        init(from decoder: Decoder) throws {
            ...
            case "object":
                ...
                let orderedProperties = try container.decodeIfPresent([String].self, forKey: .order)
                    ?? propsContainer.allKeys.map(\.stringValue).sorted()
    ...
    public init(
        ...
        let objectNode = ObjectNode(
            title: Self.displayName(for: typeName),
            description: description,
            properties: props,
            orderedProperties: properties.map(\.name),
            required: required
        )
    ...
    private static func convertDynamic(
        ...
        case .object(let name, let desc, let properties):
            ...
            let node = Node.object(
                ObjectNode(
                    title: name.map(Self.displayName(for:)),
                    description: desc,
                    properties: props,
                    orderedProperties: properties.map(\.name),
                    required: required
    ...
    private static func displayName(for typeName: String) -> String {
    ...
    public func encode(to encoder: any Encoder) throws {
        ...
        if case .ref(let refName) = root, let rootNode = defs[refName] {
            try rootNode.encode(to: encoder)
            return
        }
